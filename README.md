...
根据0-1原理，向一个双调递减序列后添1，其仍然保持双调递减（即1-0-1序列）。在进行该排序网络的比较后，b、c中至少有一个是“双调清洁的”。递归比较网络，最终可以得到有序序列。
...
### 5 性能分析
#### 5.1 时间复杂度
理论上是n(logn)^2，如果n个线程并行执行，可下降到(logn)^2。
...
**16:30-23:00** Debug不顺利，参考了别人的实现[3][4]
...
![](https://github.com/LUplus/bitonicSort/blob/master/pic/git_commit_history.png.jpg)
